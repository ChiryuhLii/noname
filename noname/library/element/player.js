...
class Player extends HTMLDivElement {
  ...
  when(triggerName, instantlyAdd = true) {
    ...
    let skill = {
      ...
      get filter() {
        return (event, player3, name2) => skill.filterFuns.every((fun) => Boolean(fun(event, player3, name2)));
      }
    };
...
${body}
  ...
  recast(cards, recastingLose, recastingGain) {
    ...
    if (typeof recastingLose != "function") {
      ...
      recastingLose = (player2, cards2) => {
        const next = player2.loseToDiscardpile(cards2);
        next.log = false;
        return next;
      };
    }
    ...
    if (typeof recastingGain != "function") {
      ...
      recastingGain = (player2, cards2) => {
        const next = player2.draw(cards2.length);
        next.log = false;
        return next;
      };
    }
