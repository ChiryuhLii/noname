  ...
  async equip(event, trigger, player) {
    ...
    if (loseCards.length) {
      ...
      for (const [key, value] of lib.commonArea) {
        const list2 = (_status[value.areaStatusName] || []).filter((card) => loseCards.includes(card));
    ...
    const handleEquip = async (card) => {
      ...
      let cardInfo2 = get.info(card, false);
      ...
      if (cardInfo2.onEquip && (!cardInfo2.filterEquip || cardInfo2.filterEquip(card, player))) {
      ...
      if (!flashAnimation) {
        ...
        if (event.createDialog && !event.dialog) {
          if (Array.isArray(event.createDialog)) {
            game.broadcastAll(
              (id, createDialog2) => {
                const dialog = ui.create.dialog.apply(void 0, createDialog2);
                dialog.videoId = id;
              },
              event.videoId,
              event.createDialog
